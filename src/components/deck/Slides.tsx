...
import { Target, Clock, TrendingUp, Lightbulb, MapPin, HelpCircle, Brain, Pin, Network as NetworkIcon, Scale, Repeat, Layers as LayersIcon, Calculator, Receipt, Table2, BarChart3, RefreshCw, FlaskConical, BookOpen, AlertTriangle, PenLine, Puzzle, FileText, GraduationCap, Ban, Check } from "lucide-react";
import { SessionIcon, ChapterIcon } from "@/components/ui/icons";
import Icon from "./Icon";
const I = (Icon: any) => <Icon size={26} strokeWidth={2.2} />;
...
/** Remove emoji from free text so icons stay consistent (Lucide only) */
const T = (s: any) => typeof s === "string" ? s.replace(EMOJI, "").replace(/\s{2,}/g, " ").trim() : s;
...
function Head({ num, title, en }: { num: React.ReactNode; title: string; en?: string }) {
  return (
    <div className="hd">
      ...
      <div {...A("l", 0.1)}><h2>{T(title)}</h2>{en && <div className="en">{en}</div>}</div>
    </div>
  );
}
const Card = ({ it, a }: { it: any; a: any }) => (
  <div className="card" {...a}><span className="ic"><Icon e={it.icon} /></span><h3>{it.t}</h3><p>{it.d}</p>{it.code && <pre className="code">{it.code}</pre>}</div>
);
...
export function TitleSlide({ s }: { s: Session }) {
  ...
  return (
    <div className="title-slide">
      <div className="icon" style={{ top: 80, left: 120 }}><SessionIcon num={s.num} className="h-[90px] w-[90px]" strokeWidth={1.2} /></div>
      <div className="icon" style={{ bottom: 100, right: 120, animationDirection: "reverse" }}><ChapterIcon ch={s.chapter} className="h-[90px] w-[90px]" strokeWidth={1.2} /></div>
      ...
      <div className="by" {...A("d", 0.7)}><GraduationCap size={22} /> إعداد وتقديم: {CREATOR} <span>{ch.en}</span></div>
    </div>
  );
}
export function EndSlide({ s }: { s: Session }) {
  return (
    <div className="end" style={{ height: "100%" }}>
      ...
      <p {...A("", 0.6)} style={{ fontWeight: 800, color: "#111", marginTop: 24, display: "flex", alignItems: "center", gap: 8, justifyContent: "center" }}><GraduationCap size={22} /> {CREATOR}</p>
...
const Hook = ({ sl, s }: P) => (
  <div className="hook"><div className="big"><SessionIcon num={s.num} className="h-[170px] w-[170px]" strokeWidth={1} /></div>
    <div className="pill" {...A("z")}>{T(sl.title)}</div>
    <div className="q" style={{ marginTop: 26 }} {...A("l", 0.3)}>{sl.q}</div>
    <div className="s" {...A("", 0.9)}>{sl.sub}</div></div>
);
const Objectives = ({ sl }: P) => (<>
  <Head num={I(Target)} title="أهداف الجلسة" en="Learning Objectives" />
  {sl.items.map((it: any, k: number) => (
    <div className="obj" key={k} {...A("l", 0.25 + k * 0.25)}><div className="n">{k + 1}</div><div className="ic"><Icon e={it.icon} /></div><div><h3>{it.t}</h3><p>{it.d}</p></div></div>
  ))}
</>);
const Timeline = ({ sl }: P) => (<>
  <Head num={I(Clock)} title={sl.title} />
  <div className="tl">{sl.steps.map((it: any, k: number) => (
    <div className="it" key={k} {...A("l", 0.25 + k * 0.22)}><div className="y">{it.y}</div><div className="ic"><Icon e={it.icon} /></div><div><h3>{it.t}</h3><p>{it.d}</p></div></div>
  ))}</div>
</>);
const Chart = ({ sl }: P) => {
  const mx = Math.max(...sl.bars.map((b: any) => b[1]));
  return (<>
    <Head num={I(TrendingUp)} title={sl.title} />
...
const Cards = ({ sl }: P) => {
  ...
  return (<>
    <Head num={I(Lightbulb)} title={sl.title} />
...
const Examples = ({ sl }: P) => (<>
  <Head num={I(MapPin)} title={sl.title} en="Real-World Examples" />
...
function Quiz({ sl, onQuiz }: P) {
  ...
  return (<>
    <Head num={I(HelpCircle)} title={sl.title} en="Interactive Quiz — دوس على الإجابة" />
    {sl.items.map((q: any, k: number) => {
      ...
      return (
        <div key={k} className={`qz${k ? " frag" : ""}${picked !== undefined ? " done" : ""}`} {...(k === 0 ? A("", 0.25) : {})}>
          ...
          <div className="why"><b>ليه؟</b> {q.why}</div>
        </div>
      );
    })}
  </>);
...
const Challenge = ({ sl }: P) => (<>
  <Head num={I(Brain)} title={sl.title} en="Think Like an Engineer" />
  <div className="ch">
    <div><div className="sc" {...A("r", 0.2)}><p>{sl.scenario}</p></div><div className="dl" {...A("", 1.4)}><b>المطلوب تسليمه:</b> {sl.deliverable}</div></div>
...
const Summary = ({ sl }: P) => (<>
  <Head num={I(Pin)} title="ملخص الجلسة" en="Session Summary" />
  <div className="sm">
    <div>{sl.points.map((p: string, k: number) => <div className="pt" key={k} {...A("l", 0.2 + k * 0.2)}><span className="ck"><Check size={16} strokeWidth={3} /></span><span>{p}</span></div>)}</div>
    <div className="side">
      ...
      <div className="next" {...A("", 1.1)}><b>الجاية:</b> {sl.next}</div>
    </div>
  </div>
</>);
const Nested = ({ sl }: P) => (<>
  <Head num={I(Target)} title={sl.title} en="AI ⊃ ML ⊃ DL ⊃ GenAI" />
  <div className="rings">
...
const Network = ({ sl }: P) => {
  ...
  return (<>
    <Head num={I(NetworkIcon)} title={sl.title} en="Artificial Neural Network" />
...
const Col = ({ c, side, k0 }: { c: any; side: string; k0: number }) => (
  <div className={`col ${c.tone || ""}`}><div className="h" {...A(side, k0 - 0.1)}>{T(c.h)}</div>
    {c.items.map(([a, b]: any, k: number) => <div className="row" key={k} {...A(side, k0 + k * 0.15)}><b>{a}</b><span>{b}</span></div>)}</div>
);
const Compare = ({ sl }: P) => (<>
  <Head num={I(Scale)} title={sl.title} />
...
const Flow = ({ sl }: P) => (<>
  <Head num={I(Repeat)} title={sl.title} />
  {sl.lead && <div className="lead" {...A("", 0.15)}>{sl.lead}</div>}
  <div className="flow">{sl.steps.map((st: any, k: number) => (
    <React.Fragment key={k}>{k > 0 && <div className="ar" {...A("", 0.35 + k * 0.3)}>←</div>}
      <div className="fs" {...A("z", 0.3 + k * 0.3)}><span className="ic"><Icon e={st.icon} /></span><h3>{st.t}</h3><p>{st.d}</p></div></React.Fragment>
  ))}</div>
</>);
const Layers = ({ sl }: P) => (<>
  <Head num={I(LayersIcon)} title={sl.title} en="Defense in Depth & Zero Trust" />
  <div className="lay">
...
const Matrix = ({ sl }: P) => {
  ...
  return (<>
    <Head num={I(Calculator)} title={sl.title} en="Risk Assessment Matrix" />
...
const Codes = ({ sl }: P) => (<>
  <Head num={I(Receipt)} title={sl.title} en="Status Codes • API • JSON" />
  <div className="cd">
...
const Table = ({ sl }: P) => (<>
  <Head num={I(Table2)} title={sl.title} en={sl.en || "Comparison Table"} />
  <table className="tb"><thead><tr {...A("", 0.15)}>{sl.head.map((h: string, i: number) => <th key={i}>{T(h)}</th>)}</tr></thead>
    <tbody>{sl.rows.map((r: string[], k: number) => <tr key={k} {...A("l", 0.3 + k * 0.18)}>{r.map((c, i) => <td key={i}>{c}</td>)}</tr>)}</tbody></table>
</>);
const Metrics = ({ sl }: P) => (<>
  <Head num={I(TrendingUp)} title={sl.title} en="Web Analytics Metrics" />
  <div className="mt">{sl.items.map((it: any, k: number) => <div className="m" key={k} {...A("z", 0.25 + k * 0.25)}><div className="ic"><Icon e={it.icon} /></div><div className="v">{it.v}</div><h3>{it.t}</h3><p>{it.d}</p></div>)}</div>
</>);
const Cycle = ({ sl }: P) => (<>
  <Head num={I(RefreshCw)} title={sl.title} en="Plan → Do → Check → Act" />
  <div className="cyc">
    <div className="wheel">{sl.steps.map((st: any, k: number) => <div className="c" key={k} {...A("z", 0.2 + k * 0.25)}><span className="ic"><Icon e={st.icon} /></span>{st.t}</div>)}<div className="mid">PDCA</div></div>
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>{sl.steps.map((st: any, k: number) => <div className="card" key={k} {...A("l", 0.4 + k * 0.25)} style={{ padding: "12px 18px" }}><h3 style={{ display: "flex", alignItems: "center", gap: 8 }}><Icon e={st.icon} size={20} /> {st.t}</h3><p>{st.d}</p></div>)}</div>
  </div>
</>);
const AB = ({ sl }: P) => {
  const V = ({ x, d }: { x: any; d: number }) => (
    <div className="v" {...A("z", d)}><div className="lb">{T(x.label)}</div>
  ...
  return (<><Head num={I(FlaskConical)} title={sl.title} en="A/B Test" /><div className="lead" {...A("", 0.1)}>{sl.lead}</div><div className="ab"><V x={sl.a} d={0.3} /><V x={sl.b} d={0.55} /></div></>);
};
const Terms = ({ sl }: P) => (<>
  <Head num={I(BookOpen)} title={sl.title || "مصطلحات أساسية من الكتاب"} en={sl.en || "Key Terms — Glossary"} />
...
const Tips = ({ sl }: P) => (<>
  <Head num={I(AlertTriangle)} title={sl.title || "خلّي بالك في الامتحان"} en={sl.en || "Exam Alerts — common traps"} />
  {sl.lead && <div className="lead" {...A("", 0.1)}>{sl.lead}</div>}
  <div className="tips">{sl.items.map((t: string, k: number) => <div className="tip" key={k} {...A("z", 0.2 + k * 0.15)}><span className="ic">{k % 2 ? <Ban size={20} /> : <AlertTriangle size={20} />}</span><div dangerouslySetInnerHTML={{ __html: t }} /></div>)}</div>
</>);
const QA = ({ sl }: P) => (<>
  <Head num={I(PenLine)} title={sl.title || "أسئلة الكتاب — أجب باختصار"} en={sl.en || "Short-answer drill — click to reveal"} />
...
const Fill = ({ sl }: P) => {
  ...
  return (<>
    <Head num={I(Puzzle)} title={sl.title || "أكمل الفراغات"} en={sl.en || "Fill in the blanks — from the textbook"} />
    <div className="fill">
...
const Essay = ({ sl }: P) => (<>
  <Head num={I(FileText)} title={sl.title || "سؤال مقالي — زي الامتحان"} en={sl.en || "Essay question with model answer"} />
  <div className="essay">
